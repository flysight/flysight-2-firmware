...
typedef struct{
  ...
  uint16_t  CustomSystemHdle;                    /**< System handle */
  uint16_t  CustomTokenHdle;                  /**< Token handle */
  uint16_t  CustomDescriptionHdle;                  /**< Description handle */
  uint16_t  CustomControlHdle;                  /**< Control handle */
...
#define COPY_SYSTEM_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_TOKEN_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_DESCRIPTION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_CONTROL_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for System +
   *                                2 for Token +
   *                                2 for Description +
   *                                2 for Control +
  ...
  COPY_SYSTEM_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomSystemHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: System, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: System \n\r");
  }

  /**
   *  Token
   */
  COPY_TOKEN_UUID(uuid.Char_UUID_128);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : TOKEN, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : TOKEN \n\r");
  }
   ...
   *  Description
   */
  COPY_DESCRIPTION_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomSystemHdle,
                          UUID_TYPE_128, &uuid,
                          SizeDescription,
                          CHAR_PROP_WRITE,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DESCRIPTION, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DESCRIPTION \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomSystemHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomControlHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CONTROL, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CONTROL \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TOKEN:
      ret = aci_gatt_update_char_value(CustomContext.CustomSystemHdle,
                                       CustomContext.CustomTokenHdle,
                                       0, /* charValOffset */
                                       SizeToken, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TOKEN command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value TOKEN command\n\r");
      }
    ...
    case CUSTOM_STM_DESCRIPTION:
      ret = aci_gatt_update_char_value(CustomContext.CustomSystemHdle,
                                       CustomContext.CustomDescriptionHdle,
                                       0, /* charValOffset */
                                       SizeDescription, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DESCRIPTION command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DESCRIPTION command\n\r");
      }
    ...
    case CUSTOM_STM_CONTROL:
      ret = aci_gatt_update_char_value(CustomContext.CustomSystemHdle,
                                       CustomContext.CustomControlHdle,
                                       0, /* charValOffset */
                                       SizeControl, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CONTROL command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CONTROL command\n\r");
      }
