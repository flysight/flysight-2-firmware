...
async def read_file(address, offset, remote_filename, local_filename, timeout=1):
    with tqdm(desc="Receiving Bytes", unit="B", unit_scale=True) as pbar:
        async with BleakClient(address, adapter='hci0') as client:
            ...
            offset_bytes = offset.to_bytes(4, byteorder='little')
            await client.write_gatt_char(CRS_RX_UUID, b'\x02' + offset_bytes + remote_filename.encode())
...
def main():
    ...
    if args.list:
        ...
        offset, remote_filename, *local_filename = args.read
        local_filename = local_filename[0] if local_filename else os.path.basename(remote_filename)
        asyncio.run(read_file(args.address, int(offset), remote_filename, local_filename))
    elif args.write:
